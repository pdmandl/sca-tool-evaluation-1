#!/usr/bin/env python3
"""Build a mixed-ecosystem OSV-based ground-truth dataset.

This script collects vulnerable package/version observations across multiple
ecosystems, enriches them with OSV-backed metadata, validates the resulting rows
against cached OSV responses, and emits a publication-ready artifact bundle.

The generated bundle consists of:
- a CSV ground-truth dataset,
- a human-readable statistics report,
- a CycloneDX SBOM,
- and a machine-readable metadata file.

Configuration is intentionally environment-driven so that experiments can be
...
def compute_global_counts(all_rows):
    ...
    """
    Compute global component and vulnerability-entry counts.

    A component is defined at the ``(ecosystem, component_name, component_version)``
...
def normalize_description(text: str) -> str:
    """
    Normalize vulnerability description text for CSV-friendly output.
    """
    """
...
def get_vulnerability_description(vuln_id: str) -> str:
    """
    Fetch and cache a short human-readable description for one vulnerability.

    The function prefers OSV summaries and falls back to the first sentence of the
    detailed description. Results are memoized in the module-level cache.
    """
    """
...
def sanitize_description(text: str, max_len: int = 240) -> str:
    """
    Shorten and normalize free-text descriptions for stable artifact output.
    """
    """
...
def _stable_row_key(r):
    """
    Build a stable canonical key for one dataset row.
    """
    return (
...
def validate_cyclonedx_sbom(bom: dict) -> None:
    """
    Validate a generated SBOM against the CycloneDX 1.5 JSON schema.
    """
    """
...
def build_sbom(
    ...
    *,
    ...
    """
    Build and optionally validate a component-centric CycloneDX SBOM.

    The SBOM is generated from the final validated dataset rows and is intended to
    be consumable by downstream tools such as Snyk or Dependency-Track.
    """
    """
...
def balance_rows_by_vulnerability_deterministic(
    ...
    """
    Balance rows across ecosystems in a deterministic way.

    The balancing procedure aims to equalize vulnerability-entry counts while
    retaining as much component diversity as possible.
    """
...
def cap_per_component(rows, max_per_component=10):
    """
    Apply a hard cap to the number of retained rows per component.
    """
    from collections import defaultdict
...
def main():
    """
    Build the final ground-truth artifact bundle from environment-based settings.
    """
    import csv
    import os
