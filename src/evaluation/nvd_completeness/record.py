...
cpeMatch`` tree of CPE applicability statements. The classifier
(:mod:`evaluation.nvd_completeness.coverage`) only ever needs a *flat* list of
those CPE match entries, each reduced to the handful of fields that matter for
product matching and (in a later slice) version-range checks. :func:`parse_nvd_record`
performs that reduction; it does no I/O and never raises on a malformed record —
an unrecognized shape simply yields no nodes (``NO_CPE_CONFIG`` downstream).
"""
...
class NvdCpeNode:
    ...
    ``None``; range bounds carry the ``versionStart*`` / ``versionEnd*``
    qualifiers verbatim. The version fields are parsed now for the next slice
    (version-precise coverage); the product matcher in this slice ignores them.
    """
...
def parse_nvd_record(raw: Any) -> Optional[ParsedNvdRecord]:
    ...
    carries no CPE configuration" (``NO_CPE_CONFIG`` downstream). Only nodes
    flagged ``vulnerable`` are kept.
    """
