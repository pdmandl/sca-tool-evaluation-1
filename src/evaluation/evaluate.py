#!/usr/bin/env python3
"""Evaluate one tool's normalized findings against a fixed ground truth.

This module is the reusable single-run evaluation entry point that is called
both directly from the command line and indirectly by the temporal runner. It
loads the ground truth, initializes the requested adapter, normalizes the tool
...
def iter_with_progress(items, *, desc: str, unit: str):
    ...
    Progress reporting is enabled only when explicitly allowed by the environment
    and when stderr is connected to an interactive terminal.
    """
    import sys
...
def _get_identifier(x: Finding) -> str:
    """
    Return the preferred vulnerability identifier for one finding.
    """
...
def _gt_key(x: Finding) -> Tuple[str, str, str, str]:
    """
    Build the canonical ground-truth-side key used for GT alignment.
    """
    """
...
def _build_gt_detection_vector(
    ...
    """
    Map confirmed true positives back to GT-order detection decisions.

    The vector is later used for significance tests and therefore must preserve the
    original ground-truth ordering and multiplicity semantics.
    """
    """
...
def _compute_gt_summary(ground_truth: List[Finding]) -> Dict[str, Dict[str, int]]:
    """
    Compute per-ecosystem ground-truth counts for reporting.
    """
...
def compute_per_ecosystem_metrics(
    *,
    ...
    """
    Compute per-ecosystem counts and derived metrics from evaluated results.
    """
    """
...
def _init_adapter(tool: str, config: dict):
    """
    Instantiate the adapter corresponding to the requested tool name.
    """
    if tool == "dtrack":
...
def run_evaluation(
    *,
    ...
    """
    Execute a single complete evaluation run for one tool.

    The function is intentionally reusable: it can write on-disk reports for
    standalone execution and also return structured payloads for orchestration by
    the temporal runner.
    """
...
def main() -> None:
    """
    Parse CLI arguments and run the single-tool evaluation workflow.
    """
