...
def write_report(
    *,
    ...
    with out_path.open("w", encoding="utf-8") as f:
        ...
        # --- False Positives (always shown, even if empty) ---
        f.write("\n\n")
        _write_false_positives_table(f, fp)

        # --- False Negatives (always shown, even if empty) ---
        f.write("\n\n")
        _write_false_negatives_table(f, fn, fn_stats)

        # --- True Positives (always shown, even if empty) ---
        f.write("\n\n")
        _write_list(
            f,
    ...
    *,
    ...
    for k, lst in fn_stats.items():
        if not lst:
            continue
        for g in lst:
            # expected: k is "FN_exact" / "FN_range" / "FN_true"
    ...
    # Safety: if FN lists are missing/empty, derive FN consistently from GT
    # (when fn_stats was not tracked at all)
