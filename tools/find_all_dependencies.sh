...
# imports_to_pyproject_with_logfile.sh
#
# Recursively scans Python files below a source directory (default: src/),
# extracts third-party imports via Python AST, filters stdlib and local modules,
# deduplicates results, maps import names to distribution names where possible,
...
# Usage:
#   ./imports_to_pyproject_with_logfile.sh
#   ./imports_to_pyproject_with_logfile.sh src
#   ./imports_to_pyproject_with_logfile.sh src poetry
#   ./imports_to_pyproject_with_logfile.sh src poetry caret
#   ./imports_to_pyproject_with_logfile.sh src pep621 exact
#
# Arguments:
#   1: source directory       (default: src)
#   2: output mode            (poetry | pep621, default: poetry)
...
#
# Logging:
#   - By default, logs are written to ./imports_scan.log
#   - Override via: LOG_FILE=/path/to/scan.log
#   - Disable file logging via: DISABLE_LOG_FILE=1
#   - Enable verbose scan logging via: DEBUG_SCAN=1
#
# Notes:
...
# Logging
# ------------------------------------------------------------
debug_enabled = os.environ.get("DEBUG_SCAN", "0").lower() in {"1", "true", "yes", "on"}
disable_log_file = os.environ.get("DISABLE_LOG_FILE", "0").lower() in {"1", "true", "yes", "on"}
log_file_path = Path(os.environ.get("LOG_FILE", "imports_scan.log")).resolve()

log = logging.getLogger("imports_to_pyproject")
log.handlers[:] = []
log.propagate = False
log.setLevel(logging.INFO if debug_enabled else logging.WARNING)
...
if debug_enabled and not disable_log_file:
...
for py_file in sorted(src_dir.rglob("*.py")):
    ...
    try:
        ...
        log.info("Could not parse file: %s (%s)", py_file, e)
        continue
...
if debug_enabled and not disable_log_file:
    print(f"# Detailed scan log written to: {log_file_path}")
elif debug_enabled and disable_log_file:
    print("# Detailed scan logging is enabled on stderr only.")
else:
    print("# Set DEBUG_SCAN=1 to enable detailed scan logging.")
PY
