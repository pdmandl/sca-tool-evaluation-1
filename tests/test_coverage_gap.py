...
class TestPrepareToolInputs:
    def test_without_sbom(self, tmp_path):
        ...
        gt = tmp_path / "gt.csv"; gt.write_text("a,b")
    ...
    def test_with_sbom(self, tmp_path):
        from evaluation.temporal_runner import prepare_tool_inputs
        gt = tmp_path / "gt.csv"; gt.write_text("x")
        sbom = tmp_path / "sbom.json"; sbom.write_text("{}")
        out_dir = tmp_path / "out"
...
def test_pre_balance_no_cve():
    ...
    rows = [{"ecosystem": "npm", "component_name": "a",
             "component_version": "1", "vulnerability_id": "OSV-1", "cve": None}]
...
def test_write_statistics_with_env_vars(tmp_path, monkeypatch):
    ...
    rows = [
        {"ecosystem": "pypi", "component_name": "a", "component_version": "1.0",
         "vulnerability_id": "OSV-1", "cve": "CVE-1", "date_published": "2024-01-01"},
    ]
